...
namespace HockeySignups.Data
{
    public class HockeySignupsDb
    {
        ...
        public Event GetLatestEvent()
        {
            ...
            InitiateDbAction(cmd =>
            {
                cmd.CommandText = "SELECT TOP 1 * FROM Events WHERE Date >= GETDATE() ORDER BY Date";
                var reader = cmd.ExecuteReader();
                if (!reader.Read())
                {
                    return;
        ...
        {
            int count = 0;
            InitiateDbAction(command =>
            {
                command.CommandText = "SELECT COUNT(*) FROM EventSignups WHERE EventId = @eventId";
